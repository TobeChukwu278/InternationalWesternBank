# Dot-Map Globe Hero Illustration Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Replace the rough hand-drawn SVG hero illustration with a polished animated dot-map globe.

**Architecture:** Single component rewrite of `src/components/features/hero-illustration.tsx`. The component renders an SVG with dot-cluster continents, faint latitude lines, financial hub arc paths, and CSS keyframe animations (pulsing nodes, flowing particles). No new files needed.

**Tech Stack:** Next.js 16, Tailwind CSS v4, inline SVG, CSS keyframes.

---

### Task 1: Write the new dot-map globe SVG (static)

**Files:**
- Modify: `src/components/features/hero-illustration.tsx` — full rewrite

- [ ] **Step 1: Replace hero-illustration.tsx with static dot-map SVG**

Replace the entire file content. The SVG viewBox is `0 0 300 300`, center at (150,150), globe radius 130.
...
Expected: "Compiled successfully" or "✓" with no TS errors.

- [ ] **Step 3: Commit**

```bash
git add src/components/features/hero-illustration.tsx
git commit -m "feat(hero): add static dot-map globe illustration"
```
...
**Files:**
- Modify: `src/components/features/hero-illustration.tsx`

- [ ] **Step 1: Add CSS animations inside `<defs>`**

Insert a `<style>` block inside `<defs>` (after the filters) with these animations:
...
<style>
  {`
    ...
    .globe-breathe { animation: breathe 6s ease-in-out infinite; }
  `}
</style>
```

- [ ] **Step 2: Add animation classes to the SVG elements**

Apply the CSS classes:

1. Wrap the entire SVG content in `<g className="globe-breathe">` (inside the SVG tag, after `<defs>`, before the ocean circle)
2. Add class `hub-large` to NYC and Tokyo nodes, `hub-small` to all other hub nodes
3. Add flow-path arcs — duplicate the arc line `<g>` as a second `<g>` with the flow classes:

Replace the existing arc lines `<g>` with this:
...
4. Add `animationDelay` to hub nodes for staggered pulsing:

```tsx
<circle cx="105" cy="90" r="4" opacity="0.9" style={{ animationDelay: "0s" }} className="hub-large" />
<circle cx="185" cy="85" r="4" opacity="0.9" style={{ animationDelay: "0.5s" }} className="hub-large" />
<circle cx="128" cy="220" r="3" opacity="0.9" style={{ animationDelay: "1s" }} className="hub-small" />
<circle cx="240" cy="70" r="4" opacity="0.9" style={{ animationDelay: "1.5s" }} className="hub-small" />
<circle cx="225" cy="140" r="3" opacity="0.9" style={{ animationDelay: "2s" }} className="hub-small" />
<circle cx="235" cy="200" r="3" opacity="0.9" style={{ animationDelay: "2.5s" }} className="hub-small" />
```
...
```bash
git add src/components/features/hero-illustration.tsx
git commit -m "feat(hero): add globe animations — pulsing hubs, flowing particles, ambient breathe"
```
...
Run: `git status`
Expected: Only the one modified file, no unrelated changes.

- [ ] **Step 3: View the result**

Start dev server with: `pnpm dev`
Open http://localhost:3000 — verify the hero section shows the animated dot-map globe with:
- Dot-cluster continents recognizable
- Faint latitude rings
- Hub nodes pulsing at financial cities
- Particles flowing along arc lines
- Gentle breathing animation on the globe

- [ ] **Step 4: Commit any final tweaks**

```bash
git add -A
git commit -m "fix(hero): adjust dot positions and animation timings"
```
